...
namespace TelergramEALLOBot.Classes
{
	public static class ResponseDataBase
	{
		public static Dictionary<RequestType, List<string>> responseTypeText = new Dictionary<RequestType, List<string>>()
		{
			...
			{ RequestType.NoSongs, new List<string>() { "Не в настроении я петь. Голос простудил, воды ж горячей нет нифига.", "Я бы спел, только мне динамики надо купить для этого."} } ,
			{ RequestType.Songs, new List<string>() { "Я вообще так себе пою, я же бот в Телеграме. Но только ради тебя, вот, послушай: " } } ,
			{ RequestType.SomethingStrange, new List<string>() { "Не знаю что и сказать. Ты хочешь странного.", "У меня голова болит.", "Я занят более важными делами: общаюсь с киевэнерго по поводу горячего всего."}  } ,
			...
			{ RequestType.NoSongsFound, new List<string>() { "Об этом песни не слагают.", "Про такое не поют.", "Пожалуйста, нет."} } ,
			{ RequestType.DoorsLocked, new List<string>() { "Ты молодец, но я вряд ли это запомню."} } ,
